...
-- | A command on this primitive processor - either nothing or an add.
...
-- | Deserialize an input into a Cmd
...
{-|
Apply a program (list of commands), and get back all the timestamps when
the register changes value.  One can then do point in time queries against the
historical state of the register.
-}
...
{-|
Get from a map the value assosciated with the greatest key not greater than the
given argument.
-}
